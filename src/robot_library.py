...
class Move():
    ...
    def MoveJ(self, pos, v = None, a = None, Origin = None):
        ...
        command = f"MoveJ X{X} Y{Y} Z{Z} y{y} p{p} r{r} s{v} a{a}\n"
    ...
    def OffsetJ(self, pos, v = None, a = None):
        X = pos[0]
        Y = pos[1]
        Z = pos[2]  
        y = pos[3]
        ...
        command = f"OffsetJ X{X} Y{Y} Z{Z} y{y} p{p} r{r} s{v} a{a}\n"
    ...
    def JogJoint(self, pos, v = None, a = None):
        ...
        command = f"jogJ J1{J1} J2{J2} J3{J3} J4{J4} J5{J5} J6{J6} s{v} a{a}\n"
    ...
    def MoveL(self, pos, a = None, v = None, Origin = None):
        if Origin:
            X = pos[0] + Origin[0]
            Y = pos[1] + Origin[1]
            Z = pos[2] + Origin[2]
        ...
        command = f"MoveL X{X} Y{Y} X{Z} X{y} X{p} X{r} s{v} a{a}\nEnd\n"
    ...
    def OffsetL(self, pos, a = None, v = None):
        X = pos[0]
        Y = pos[1]
        Z = pos[2]  
        y = pos[3]
        ...
        command = f"OffsetL X{X} Y{Y} Z{Z} y{y} p{p} r{r} s{v} a{a}\nEnd\n"
    ...
    def MoveJointPos(self, pos):
        ...
        command = f"MoveJoint J1{J1} J2{J2} J3{J3} J4{J4} J5{J5} J6{J6} s50 a50\nEnd\n"
    ...
    def __init__(self, tool):
        ...
        for i in range(len(var.TOOLS3D)):
            if tool == os.path.splitext(var.TOOLS3D[i][0])[0]:
                ...
                settings_tool = ("Set_tool A" + str(tool_pin) + "B" + str(type_tool) + "C" + str(var.TOOLS3D[self.tool_number][8][0]) + "D" + str(var.TOOLS3D[self.tool_number][8][1]) + "\n")      
                print(settings_tool)
                ...
                if var.SETTINGS['Set_tools'][1] == "IO" and var.IO_CONNECT:
                    ...
                    self.TalkWithRobot.send_to_robot_one_line(settings_tool)
                    self.IO_BOX = False
                    print("no io box")                       
                
                if var.ROBOT_CONNECT:
                    self.TalkWithRobot.send_to_robot_one_line(tool_frame)
                    print(tool_frame)
...
class IO():
    def __init__(self, number, type):
        ...
        if self.type == "INPUT" or self.type == "OUTPUT":
            ...
            if var.SETTINGS["Set_io_pin"][1] == "IO" and var.IO_CONNECT:
                self.TalkWithIO.send_to_IO(settings_io)
                self.IO_BOX = True
                print("IO box")
            elif var.ROBOT_CONNECT:
                self.TalkWithRobot.send_to_robot_one_line(settings_io)
                self.IO_BOX = False
                print("no io box")  
        else:
    ...
    def digitalRead(self):
        if var.SIM and self.type == "INPUT":
            
            IO_CHECK = event_manager.publish("request_check_io_state", self.IO_number)
            
            if IO_CHECK and not var.IO_CONNECT:
                return True
            elif not var.IO_CONNECT:
                return False             
        
        if self.type == "INPUT" and var.IO_CONNECT:
            # Here the IO module will look for 0.1s if there is an signal
            # send a command so that the io knows which io port to read
            command = f"IO_read_data ({self.IO_number})\n"
            ...
            while (time.time() - start_time) < 0.1:
                command = self.TalkWithIO.data_read(self.IO_number)
                if command == 1:
                    print("got a signal yes!!!!!!!!!!!!!!!!!!!!!")
                    break   
            return command
        ...
        for line in lines_gcode:
            ...
            if X != 0 or Y != 0 or Z != 0:
                ...
                if words:
                    ...
                    if first_word_1 == "G01":
                        command = f"MoveL X{X} Y{Y} Z{Z} y{y} p{p} r{r} s{5} a{5}"                                                         
                        new_command.append(command + "\n")
                        if var.SIM:
                            self.move.MoveL([X,Y,Z,y,p,r], 10 , 10)
